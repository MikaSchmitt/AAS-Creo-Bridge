# tests/aasx_adapter/test_importer.py
...
import aas_adapter.importer as importer
from aas_adapter import AASXImportResult, import_aasx
...
def test_import_aasx_success_happy_path(
    ...
    monkeypatch.setattr(importer, "DictObjectStore", lambda: fake_object_store)
    monkeypatch.setattr(
        importer, "DictSupplementaryFileContainer", lambda: fake_file_store
    )

    # Force shell discovery to a known result (unit-testing import_aasx wiring, not shell parsing here).
    monkeypatch.setattr(
        importer, "_discover_shells", lambda identifiers, objects: ["DISCOVERED"]
    )
    ...
    monkeypatch.setattr(importer, "AASXReader", _reader_factory)

    result = import_aasx(aasx_path)

    assert isinstance(result, AASXImportResult)
...
def test_import_aasx_wraps_reader_errors_as_valueerror_and_logs(
    ...
    monkeypatch.setattr(importer, "DictObjectStore", lambda: object())
    monkeypatch.setattr(
        importer, "DictSupplementaryFileContainer", lambda: object()
    )
...
def test_discover_shells_returns_empty_when_no_identifiers() -> None:
    ...
    assert importer._discover_shells(set(), _Store()) == []
...
def test_discover_shells_filters_only_aas_shell_instances(
    ...
    monkeypatch.setattr(importer.model, "AssetAdministrationShell", _FakeShell)
    ...
    out = importer._discover_shells({"id-shell", "id-not-shell"}, _Store())
    assert out == ["id-shell"]
